...
8. ❌ **Координаты меню на Astra отличаются от Windows** — на Astra пункт
   «Настройки» на x=440, y=38 (на Windows x=476, y=34). Строка меню ниже,
   пункты расположены левее. Решение: никогда не копировать координаты между
   ОС — каждая требует своей разведки через скриншот + OCR.

9. ❌ **Тёмная тема диалогов на Astra (fly)** — на Windows текст тёмный на
   светлом фоне, на Astra — светлый текст на тёмном фоне (или наоборот).
   Поиск текста `arr[y, x, 0] < 160` находит тёмные пиксели; для светлого
   текста нужно `arr[y, x, 0] > 200`. Решение: смотреть цветовую схему ОС —
   на Astra fly-диалоги инвертированы относительно Windows.

10. ❌ **OCR не находит текст в тёмной теме** — Tesseract с --psm 7 не
    распознаёт строки в dark-theme диалогах Astra, потому что контраст между
    текстом и фоном меньше. Решение: использовать --psm 4 на всём блоке
    диалога (работает), или увеличивать контраст перед OCR.

11. ❌ **Диалог «Настройки быстрой съемки» разной высоты на разных ОС** —
    на Windows кнопки OK/Отмена на y=775-795, на Astra на y=735-755.
    Решение: каждый параметр проверять отдельной областью (не полагаться на
    единый layout).

12. ❌ **Клик по OK уходил мимо из-за неверной Y-координаты** — на Windows
    сначала указал y=788, а кнопка на y=763-786 (центр 775). На Astra центр
    кнопки на y=745. Решение: всегда сканировать точные границы кнопки:
    найти Y с текстом, взять центр.

13. ❌ **Сложность нахождения текстовых строк в диалоге Astra** — из-за
    равномерного светлого фона (RGB 239,239,239) весь диалог определяется
    как один большой текстовый блок. Решение: сканировать по тем самым
    «тёмным пикселям» (value < 200) на светлом фоне — они редкие, но только
    там, где реально есть текст или рамки.

14. ❌ **Клик по «Настройки» не открывал меню** — метод `glide_click((440, 38))` из TC-97 не срабатывал на текущем состоянии ВМ. Причина: курсор не попадал точно в строку меню из-за смещения окна или разрешения. Решение: использовать OCR, чтобы найти точные координаты текста «Настройки» на скриншоте, и кликать по ним динамически.

15. ❌ **Координаты пункта меню неизвестны заранее** — «Настройки автосъемки» находится в выпадающем меню, и его позиция зависит от версии приложения. Решение: открыть меню «Настройки», сделать скриншот, найти текст «автосъемки» через OCR, кликнуть по найденным координатам.

16. ❌ **OCR не всегда видит текст на тёмном фоне** — в выпадающем меню Astra тёмный фон, а текст светлый. Tesseract с --psm 6 не распознаёт отдельные строки. Решение: использовать `--psm 4` на всём блоке меню или улучшать контраст перед OCR (CLAHE, адаптивная бинаризация).

17. ❌ **Создание эталонов «вслепую» не даёт результата** — если запустить тест без эталонов, он создаёт их bootstrap-ом из первого же кадра, который может быть нестабильным (окно ещё не дорисовано). Решение: сначала вручную (или через временный тест) открыть окно, дождаться его стабилизации, затем снять эталоны через `capture_region` и скопировать их в `baseline/`.

18. ❌ **Порядок тестов в цепочке важен** — тест `test_astra_menu_auto_capture.py` должен идти после TC-97 (меню быстрой съёмки), потому что он полагается на стартовое состояние: приложение открыто, все диалоги закрыты. Алфавитная сортировка файлов определяет порядок: `menu_auto_capture` > `menu_quick_settings`. Переименование файла сломает цепочку.

19. ❌ **Временные тесты — нормальный инструмент разведки** — создавать отдельные временные тесты для поиска координат и создания эталонов — эффективно. Они не должны попадать в git. После получения данных их можно удалить.

20. ❌ **Всегда проверять скриншот глазами** — тест может пройти (все проверки OK), но на самом деле на скриншоте другое окно или пустой экран. Решение: после успешного прогона открывать скриншот и убеждаться, что он соответствует ожидаемому состоянию.

21. ❌ **Tesseract требует точной настройки PSM** — для распознавания отдельных строк меню лучше `--psm 7` (одна строка), для целого диалога — `--psm 4` (целый блок). Подбор режима — итеративный процесс.

## 14. Быстрый старт после перерыва
