"""UI-тест окна «Настройки автосъемки» Altami Studio на Windows.

Стартовое состояние — конечное состояние TC-94: приложение уже запущено,
окно «Настройки быстрой съемки» закрыто, на экране главное окно.
...
4. Проверяем все параметры окна: Путь, Формат, Качество, Сохранять фигуры,
   Префикс, Использовать текущую дату (год, месяц, день), В имени, Файла,
   Папка, Разделять.
5. Нажимаем кнопку ОК — окно закрывается, тест завершён.

Приложение НАМЕРЕННО остаётся открытым в конце — как в TC-85, TC-86, TC-92
и TC-94: на нём строятся следующие сценарии.
...
в алфавитном порядке, поэтому цепочка складывается правильно: запуск (TC-85) ->
активация (TC-86) -> окно «О программе» (TC-92) -> быстрая съемка (TC-94) ->
этот тест. Переименование файла порядок сломает.

Координаты и области сняты на живой ВМ 25.08.2026 в видеорежиме 1920x1200
...
AUTO_CAPTURE_ITEM = (476, 183)        # «Настройки автосъемки» в меню «Настройки»
OK_BUTTON = (1040, 786)               # середина кнопки ОК (текст x=1002-1078, y=786)
CANCEL_BUTTON = (1125, 786)           # середина кнопки Отмена (текст x=1088-1162, y=786)
MOUSE_PARK = (1750, 250)              # нейтральная точка: увести курсор из кадра

# --- Области для сравнения с эталоном (left, top, right, bottom) -------------
# Заголовок окна «Настройки автосъемки». Доказывает, что окно открылось, и
# по нему же видно, что оно закрылось после OK.
AUTO_CAPTURE_TITLE_BOX = (720, 340, 1140, 352)
...
# текстовой метки. Все области сняты на живой ВМ 25.08.2026.
...
SETTING_DATE_YEAR_BOX = (708, 623, 830, 633)
# Месяц — чекбокс под текущей датой
SETTING_DATE_MONTH_BOX = (830, 623, 950, 633)
# День — чекбокс под текущей датой
SETTING_DATE_DAY_BOX = (1100, 623, 1200, 633)
# В имени — чекбокс/поле для настройки имени файла
SETTING_IN_NAME_BOX = (705, 657, 870, 672)
# Файла — поле с именем
SETTING_FILENAME_BOX = (705, 683, 755, 693)
# Папка — поле с путём к папке
SETTING_FOLDER_BOX = (705, 683, 870, 693)
# Разделять — чекбокс
SETTING_SEPARATOR_BOX = (705, 734, 870, 747)
...
@pytest.mark.app
class TestWindowsMenuAutoCapture(BaseVMTest):
    ...
    async def _require_start_state(self) -> None:
        ...
        Стартовое состояние — конечное состояние TC-94: Altami Studio открыт,
        ...
        message.append(
            "  Подготовьте состояние вручную (или прогоном TC-85, TC-86, TC-92 и TC-94) "
            "и запустите снова."
        )
        pytest.fail("\n".join(message))
    ...
    async def test_auto_capture_settings(self):
        ...
        # 0. Стартовое состояние TC-94: главное окно Altami Studio открыто.
        ...
        await self._park_mouse()
        await self.assert_region("altami_auto_capture_title", AUTO_CAPTURE_TITLE_BOX)
